    ...
    with app.app_context():
        if not all((
            ...
            if not org:
                ...
                user_dict = {
                    'gitee_id': app.config.get("PUBLIC_USER_ID"),
                    'gitee_login': app.config.get("PUBLIC_USER_ACCOUNT"),
                    'current_org_id': org.id
                }
                user = User.query.filter_by(gitee_id=user_dict.get("gitee_id")).first()
                if not user:
                    user = User()
                    user.gitee_id = int(user_dict.get("gitee_id"))
                    user.gitee_login = user_dict.get("gitee_login")
                    user.gitee_name = user_dict.get("gitee_login")
                    user.add_update()

                re_user_org = ReUserOrganization.query.filter_by(
                    user_gitee_id=user_dict.get("gitee_id"),
                    organization_id=org.id,
                    is_delete=False
                ).first()
                if not re_user_org:
                    re_user_org = ReUserOrganization()
                    re_user_org.user_gitee_id = user_dict.get("gitee_id")
                ...
                if group_name:
                    ...
                    if re_group:
                        ...
                        if not re_user_group:
                            ...
                            re_user_group.user_gitee_id = user_dict.get("gitee_id")
                ...
                redis_client.hmset(RedisKey.user(user_dict.get('gitee_id')), user_dict, ex=expire_time)
                token = generate_token(
                    user_dict.get('gitee_id'),
                    user_dict.get('gitee_login'),
                    ex=expire_time
                )
                redis_client.set(RedisKey.token(user_dict.get('gitee_id')), token, ex=expire_time)
