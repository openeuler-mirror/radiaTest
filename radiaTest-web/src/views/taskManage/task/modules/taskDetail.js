...
const showFooterContent = ref('comment'); // 任务详情底部显示内容，默认：关联用例
...
function getTaskComment() {
  axios
    ...
    .catch((errors) => {
      window.$message?.error(
        errors.data.error_msg || errors.message || '未知错误'
      );
    });
}
        ...
        return [
          ...
          h(
            NButton,
            {
              ...
              text: true,
              disabled:
                !editStatus.value || modalData.value.detail.is_single_case,
              onClick: () => {
                if (editStatus.value) {
                  deleteCase(rowData);
                }
...
function handleSearchFatherTask(query) {
  ...
  timer = setTimeout(() => {
    familyTaskOperator({ not_in: true, is_parent: true, title: query }).then(
      (res) => {
...
function handleSearchChildTask(query) {
  ...
  timer = setTimeout(() => {
    familyTaskOperator({ not_in: true, is_parent: false, title: query }).then(
      (res) => {
