<template>
  <div class="codehub">
    <div class="search">
      ...
      <n-icon size="20" color="#666666" class="refreshIcon" @click="getCodeData">
        <md-refresh />
      </n-icon>
  ...
  <n-modal v-model:show="repoModal" preset="dialog" title="注册代码仓" @close="closeRepoForm">
    <n-form ref="repoRef1" label-placement="top" :model="repoForm" :rules="repoRules">
      ...
      <n-form-item label="名称" path="name">
        <n-input v-model:value="repoForm.name" placeholder="请输入名称" />
      </n-form-item>
    ...
    <template #action>
      <n-space style="width: 100%">
        <n-button type="error" ghost size="large" @click="closeRepoForm"> 取消 </n-button>
        <n-button size="large" @click="submitRepoForm" type="primary" ghost> 提交 </n-button>
      </n-space>
    </template>
  </n-modal>
</template>
...
function getCodeData() {
  ...
  getScopedGitRepo(params).then((res) => {
    res.data?.map((item) => {
...
const repoRules = {
  ...
  git_url: {
    ...
    validator(rule, value) {
      ...
      } else if (!/^http(s)?:\/\/[a-z0-9-]+(.[a-z0-9-]+)*(:[0-9]+)?(\/.*)?$/.test(value)) {
